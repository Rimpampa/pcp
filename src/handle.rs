...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Channel => write!(f, "Inner threads communication error"),
...
impl<Ip: IpAddress> Handle<Ip> {
    ...
    pub async fn renew(&mut self, id: usize, lifetime: u32) -> bool {
        self.to_client
    ...
    pub async fn revoke(&mut self, id: usize) -> bool {
        self.to_client.send(ServerEvent::Revoke(id)).await.is_ok()
    }
    ...
    pub async fn shutdown(self) -> bool {
        self.to_client.send(ServerEvent::Shutdown).await.is_ok()
    }
    ...
    pub async fn request<R: Map<Ip>>(&self, id: usize, map: R, kind: RequestKind) -> bool {
        self.to_client.send(map.into_event(id, kind)).await.is_ok()
    }

    pub async fn wait(&mut self) -> Option<ClientEvent<Ip>> {
