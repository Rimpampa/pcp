...
impl ResultCode {
    pub const fn explain(&self) -> &'static str {
        use ResultCode::*;
        // NOTE: this strings are copied from the docs
        match self {
            Success => "Success",
            UnsuppVersion => "The version number at the start of the PCP Request header is, not recognized by this PCP server",
            NotAuthorized => "The requested operation is disabled for this PCP client, or, the PCP client requested an operation that cannot be fulfilled by the PCP server's security policy",
            ...
            NetworkFailure => "The PCP server or the device it controls is experiencing a network failure of some sort",
            NoResources => "Request is well-formed and valid, but the server has insufficient resources to complete the requested operation at this time",
            UnsuppProtocol => "Unsupported transport protocol",
            UserExQuota => "This attempt to create a new mapping would exceed this subscriber's port quota",
            CannotProvideExternal => "The suggested external port and/or external address cannot be provided",
            AddressMismatch => "The source IP address of the request packet does not match the contents of the PCP Client's IP Address field, due to an unexpected NAT on the path between the PCP client and the PCP-controlled NAT or firewall",
            ExcessiveRemotePeers => "The PCP server was not able to create the filters in this request",
        }
    }
}
...
impl util::Deserialize for ResultCode {
    fn deserialize(data: &mut Deserializer) -> util::Result<Self> {
        match data.deserialize()? {
            ...
            n => Err(Error::UnknownResultCode(n)),
        }
    }
}
