...
use super::{payload::Payload, util, Error, OpCode, ProtocolNumber};
use util::{Deserializer, Serializer};

/// Maximum size a PCP packet can have
pub const MAX_PACKET_SIZE: usize = 1100;

// const MAX_RESPONSE_PAYLOAD_SIZE: usize = MAX_PACKET_SIZE - ResponseHeader::SIZE;
...
const fn max_options_size(payload_size: usize) -> usize {
    MAX_PACKET_SIZE - Request::HEADER_SIZE - payload_size
}
...
impl util::Deserialize for Request {
    fn deserialize(data: &mut Deserializer) -> util::Result<Self> {
        ...
        let opcode = data.deserialize()?;
        if opcode & 0b10000000 != 0 {
            return Err(Error::NotAResponse);
        }
        let opcode = u8::try_into(opcode)?;
